...
def save_dummy(cur, conn, data):
    try:
        insert_query = """
            INSERT INTO employee3 (id, nik, email, password, full_name, birth_date, department, location, division, position, education_major, education_institute, company_history, position_history, session, otp_code, otp_expiration, last_activity, session_exp)
            VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s,%s, %s, %s, %s, %s, %s);
        """
        # Siapkan data untuk di-insert
        data_to_insert = [
            (row['id'], row['nik'], row['email'],row['password'],row['full_name'],row['birth_date'],row['department'],row['location'], row['division'], row['position'],row['education_major'],row['education_institute'],row['company_history'],row['position_history'],row['session'],row['otp_code'],row['otp_expiration'],row['last_activity'],row['session_exp'])  # Hubungkan dengan id di tabel posts
            for row in data
        ]
